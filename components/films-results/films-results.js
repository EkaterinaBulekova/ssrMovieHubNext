...
const FilmsResults = (props: { count: number, sortBy: string, onClick: any }) => {
  const group = {
  ...
  const { count, sortBy, onClick } = props;
  return (
    <div className = {styles.films_results}>
      {
        (count)
          ? <React.Fragment>
            <div className={styles.films_count}>{`${count} movies found`}</div>
            <div className={styles.films_sortby}>
              <div className={styles.films_sortby_title}>{group.title}</div>
              {group.buttons.map(button => <Button key={button.result}
                className={(button.result === sortBy)
